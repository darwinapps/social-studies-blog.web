...
<?php
// Above of this tag `<?php` is placed the little snippet of Google Analytics with the tracking code correspond.
// Test with Docker this must to be removed. 
// In live page it works correctly

// Enable the SVG files support.
...
function my_theme_scripts() {
	...
	wp_enqueue_style( 'ss-blog-style', get_stylesheet_uri(), array(), '1.3' );
...
function class_menu_bootstrap($atts, $item, $args){
    $class = 'nav-link text-danger font-left35-regular font-size-menu-link mx-3 px-0 py-0 pt-2';
    $atts['class'] = $class;
    return $atts;
}
...
function footer_sidebars() {
    ...
    register_sidebar(
        array(
            ...
            'before_widget' => '<div id="%1$s" class="widget %2$s my-1 col-12 d-flex justify-content-start flex-row px-0">',
            'after_widget'  => '</div>',
            'before_title'  => '<div>',
            'after_title'   => '</div>',
        )
